...
class FirestoreDelegate extends StorageDelegate {
  ...
  @override
  Future<DocData?> getDocument(String collection, String document) => storage
      .collection(prefix + collection)
      .doc(document)
      .get()
      .then((snap) => snap.data());
  ...
  Future<void> create(String collection, String document, DocData value) =>
      storage
          .collection(prefix + collection)
          .doc(document)
          .set(value, fs.SetOptions(merge: true));
  ...
  StreamSubscription<List<DocData>> collectionListener(
    ...
    return asCollectionStream(
            storage.collection(prefix + collection).snapshots())
        .listen(
      onData,
      onError: onError,
      onDone: onDone,
  ...
  StreamSubscription<DocData?> documentListener(
    ...
    return asDocumentStream(
            storage.collection(prefix + collection).doc(document).snapshots())
        .listen(
      onData,
      onError: onError,
      onDone: onDone,
  ...
  Stream<DocData?> asDocumentStream(
      Stream<DocumentSnapshot<DocData?>> source) async* {
    try {
  ...
  Stream<List<DocData>> asCollectionStream(
      Stream<QuerySnapshot<DocData>> source) async* {
    try {
